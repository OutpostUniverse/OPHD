...
void MapViewState::deploySeedLander(NAS2D::Point<int> point)
{
	...
	// TOP ROW
	Structure* sp = StructureCatalogue::get(StructureTypeID::SID_SEED_POWER);
	structureManager.addStructure(sp, &mTileMap->getTile(point + DirectionNorthWest));
	...
	// BOTTOM ROW
	SeedFactory* sf = static_cast<SeedFactory*>(StructureCatalogue::get(StructureTypeID::SID_SEED_FACTORY));
	...
	SeedSmelter* ss = static_cast<SeedSmelter*>(StructureCatalogue::get(StructureTypeID::SID_SEED_SMELTER));
