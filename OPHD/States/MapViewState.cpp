...
MapViewState::~MapViewState()
{
	...
	Utility<Renderer>::get().setCursor(static_cast<int>(PointerType::POINTER_NORMAL));
...
void MapViewState::placeRobot()
{
	...
	if(mCurrentRobot == RobotType::ROBOT_DOZER)
	{
		...
		else if (tile->index() == static_cast<int>(TerrainType::TERRAIN_DOZED))
		{
		...
		tile->index(static_cast<int>(TerrainType::TERRAIN_DOZED));
...
void MapViewState::setStructureID(StructureID type, InsertMode mode)
{
	...
	mInsertMode = mode;
	Utility<Renderer>::get().setCursor(static_cast<int>(PointerType::POINTER_PLACE_TILE));
