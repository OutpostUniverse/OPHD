...
void MapViewState::onKeyDown(NAS2D::EventHandler::KeyCode key, NAS2D::EventHandler::KeyModifier mod, bool /*repeat*/)
{
	...
	auto filteredKeyModifier = NAS2D::EventHandler::KeyModifier::None;

	if (NAS2D::EventHandler::control(mod))
	{
		filteredKeyModifier = filteredKeyModifier | NAS2D::EventHandler::KeyModifier::Ctrl;
	}
	if (NAS2D::EventHandler::alt(mod))
	{
		filteredKeyModifier = filteredKeyModifier | NAS2D::EventHandler::KeyModifier::Alt;
	}
	if (NAS2D::EventHandler::shift(mod))
	{
		filteredKeyModifier = filteredKeyModifier | NAS2D::EventHandler::KeyModifier::Shift;
	}

	auto commandIterator = mKeyCodeMap.find({filteredKeyModifier, key});
	if (commandIterator != mKeyCodeMap.end())
	{
		commandIterator->second(*this);
	}
}
