...
namespace
{
	const NAS2D::Image& robotImage(RobotType robotType)
	{
		...
		{
			{RobotType::ROBOT_DIGGER, &imageCache.load("ui/interface/product_robodigger.png")},
			{RobotType::ROBOT_DOZER, &imageCache.load("ui/interface/product_robodozer.png")},
			{RobotType::ROBOT_MINER, &imageCache.load("ui/interface/product_robominer.png")}
		};
...
void RobotInspector::init()
{
	...
	const int imgHeight = robotImage(RobotType::ROBOT_DIGGER).size().y + padding;

	const int largestHeight = imgHeight > contentHeight ? imgHeight : contentHeight;

	size({ buttonWidth + robotImage(RobotType::ROBOT_DIGGER).size().x + padding + constants::MARGIN, 300 });
