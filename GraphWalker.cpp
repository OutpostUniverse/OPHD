...
bool CheckSourceTubeAlignment(Structure* src, Direction _d);
...
bool validConnection(Structure* src, Structure* dst, Direction _d)
{
	...
	else if (dst->isConnector())
	{
		...
		{
			// We're good if this is a (non-tube) building
			if (!src->isConnector())
			{
				return true;
			}
			// Otherwise we need to check the source tile and make sure the tube faces the right direction.
			else
			{
